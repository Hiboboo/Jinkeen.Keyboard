...
public class KeyboardShadowContainerView extends FrameLayout {
    ...
    //KeyboardShadowContainerView的样式，是只需要pressed还是selected。默认是pressed.
    ...
    public KeyboardShadowContainerView(Context context) {
        this(context, null);
    }

    public KeyboardShadowContainerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public KeyboardShadowContainerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setLayoutBackground(int color) {
        if (layoutBackground_true != null) {
            throw new UnsupportedOperationException("使用了KeyboardShadowContainerView_hl_layoutBackground_true属性，要与KeyboardShadowContainerView_hl_layoutBackground属性统一为颜色");
        }
    ...
    public void setLayoutBackgroundTrue(int color) {
        if (layoutBackground != null) {
            throw new UnsupportedOperationException("使用了KeyboardShadowContainerView_hl_layoutBackground属性，要与KeyboardShadowContainerView_hl_layoutBackground_true属性统一为颜色");
        }
    ...
    protected void onFinishInflate() {
        ...
        if (mTextViewResId != -1) {
            ...
            if (mTextView == null) {
                throw new NullPointerException("KeyboardShadowContainerView找不到hl_bindTextView，请确保绑定的资源id在KeyboardShadowContainerView内");
            } else {
                if (textColor == -101) {
        ...
        if (firstView == null) {
            firstView = KeyboardShadowContainerView.this;
    ...
    private void initAttributes(AttributeSet attrs) {
        TypedArray attr = getContext().obtainStyledAttributes(attrs, R.styleable.KeyboardShadowContainerView);
        if (attr == null) {
            return;
        }
        ...
        try {
            //默认是显示
            isShowShadow = !attr.getBoolean(R.styleable.KeyboardShadowContainerView_hl_shadowHidden, false);
            leftShow = !attr.getBoolean(R.styleable.KeyboardShadowContainerView_hl_shadowHiddenLeft, false);
            rightShow = !attr.getBoolean(R.styleable.KeyboardShadowContainerView_hl_shadowHiddenRight, false);
            bottomShow = !attr.getBoolean(R.styleable.KeyboardShadowContainerView_hl_shadowHiddenBottom, false);
            topShow = !attr.getBoolean(R.styleable.KeyboardShadowContainerView_hl_shadowHiddenTop, false);
            mCornerRadius = attr.getDimension(R.styleable.KeyboardShadowContainerView_hl_cornerRadius, 0);
            mCornerRadius_leftTop = attr.getDimension(R.styleable.KeyboardShadowContainerView_hl_cornerRadius_leftTop, -1);
            mCornerRadius_leftBottom = attr.getDimension(R.styleable.KeyboardShadowContainerView_hl_cornerRadius_leftBottom, -1);
            mCornerRadius_rightTop = attr.getDimension(R.styleable.KeyboardShadowContainerView_hl_cornerRadius_rightTop, -1);
            mCornerRadius_rightBottom = attr.getDimension(R.styleable.KeyboardShadowContainerView_hl_cornerRadius_rightBottom, -1);

            //默认扩散区域宽度
            mShadowLimit = attr.getDimension(R.styleable.KeyboardShadowContainerView_hl_shadowLimit, 0);
            if (mShadowLimit == 0) {
            ...
            //x轴偏移量
            mDx = attr.getDimension(R.styleable.KeyboardShadowContainerView_hl_shadowOffsetX, 0);
            //y轴偏移量
            mDy = attr.getDimension(R.styleable.KeyboardShadowContainerView_hl_shadowOffsetY, 0);
            mShadowColor = attr.getColor(R.styleable.KeyboardShadowContainerView_hl_shadowColor, Color.BLACK);

            selectorType = attr.getInt(R.styleable.KeyboardShadowContainerView_hl_shapeMode, 1);
            isSym = attr.getBoolean(R.styleable.KeyboardShadowContainerView_hl_shadowSymmetry, true);

            //背景颜色的点击(默认颜色为白色)
            mBackGroundColor = Color.WHITE;

            Drawable background = attr.getDrawable(R.styleable.KeyboardShadowContainerView_hl_layoutBackground);
            if (background != null) {
            ...
            Drawable trueBackground = attr.getDrawable(R.styleable.KeyboardShadowContainerView_hl_layoutBackground_true);
            ...
            if (mBackGroundColor_true != -101 && layoutBackground != null) {
                throw new UnsupportedOperationException("使用了KeyboardShadowContainerView_hl_layoutBackground_true属性，必须先设置KeyboardShadowContainerView_hl_layoutBackground属性。且设置颜色时，必须保持都为颜色");
            }

            if (layoutBackground == null && layoutBackground_true != null) {
                throw new UnsupportedOperationException("使用了KeyboardShadowContainerView_hl_layoutBackground_true属性，必须先设置KeyboardShadowContainerView_hl_layoutBackground属性。且设置图片时，必须保持都为图片");
            }


            //边框颜色的点击
            stroke_color = attr.getColor(R.styleable.KeyboardShadowContainerView_hl_strokeColor, -101);
            stroke_color_true = attr.getColor(R.styleable.KeyboardShadowContainerView_hl_strokeColor_true, -101);

            if (stroke_color == -101 && stroke_color_true != -101) {
                throw new UnsupportedOperationException("使用了KeyboardShadowContainerView_hl_strokeColor_true属性，必须先设置KeyboardShadowContainerView_hl_strokeColor属性");
            }

            stroke_with = attr.getDimension(R.styleable.KeyboardShadowContainerView_hl_strokeWith, dip2px(1));
            //规定边框长度最大不错过7dp
            if (stroke_with > dip2px(7)) {
                stroke_with = dip2px(5);
            }
            ...
            Drawable clickAbleFalseBackground = attr.getDrawable(R.styleable.KeyboardShadowContainerView_hl_layoutBackground_clickFalse);
            ...
            startColor = attr.getColor(R.styleable.KeyboardShadowContainerView_hl_startColor, -101);
            centerColor = attr.getColor(R.styleable.KeyboardShadowContainerView_hl_centerColor, -101);
            endColor = attr.getColor(R.styleable.KeyboardShadowContainerView_hl_endColor, -101);
            if (startColor != -101) {
                //说明设置了渐变色的起始色
                if (endColor == -101) {
                    throw new UnsupportedOperationException("使用了KeyboardShadowContainerView_hl_startColor渐变起始色，必须搭配终止色KeyboardShadowContainerView_hl_endColor");
                }
            }


            angle = attr.getInt(R.styleable.KeyboardShadowContainerView_hl_angle, 0);
            if (angle % 45 != 0) {
            ...
            if (selectorType == 3) {
                ...
                if (mBackGroundColor == -101 || mBackGroundColor_true == -101) {
                    throw new NullPointerException("使用了KeyboardShadowContainerView的水波纹，必须设置使用了KeyboardShadowContainerView_hl_layoutBackground和使用了KeyboardShadowContainerView_hl_layoutBackground_true属性，且为颜色值");
                }
            ...
            mTextViewResId = attr.getResourceId(R.styleable.KeyboardShadowContainerView_hl_bindTextView, -1);
            textColor = attr.getColor(R.styleable.KeyboardShadowContainerView_hl_textColor, -101);
            textColor_true = attr.getColor(R.styleable.KeyboardShadowContainerView_hl_textColor_true, -101);
            text = attr.getString(R.styleable.KeyboardShadowContainerView_hl_text);
            text_true = attr.getString(R.styleable.KeyboardShadowContainerView_hl_text_true);

            isClickable = attr.getBoolean(R.styleable.KeyboardShadowContainerView_clickable, true);
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (getChildAt(0) != null) {
            if (mCornerRadius_leftTop == -1 && mCornerRadius_leftBottom == -1 && mCornerRadius_rightTop == -1 && mCornerRadius_rightBottom == -1) {
                //说明没有设置过任何特殊角、且是半圆。
                if (mCornerRadius > trueHeight / 2.0f) {
                    Path path = new Path();
                    path.addRoundRect(rectf, trueHeight / 2.0f, trueHeight / 2.0f, Path.Direction.CW);
        ...
        if (mCornerRadius_leftTop == -1 && mCornerRadius_leftBottom == -1 && mCornerRadius_rightTop == -1 && mCornerRadius_rightBottom == -1) {
            if (mCornerRadius > trueHeight / 2.0f) {
                if (selectorType != 3) {
                    if (layoutBackground == null && layoutBackground_true == null) {
                        //画圆角矩形
                        canvas.drawRoundRect(rectf, trueHeight / 2.0f, trueHeight / 2.0f, paint);
